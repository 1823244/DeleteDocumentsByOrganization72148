			...
			Пока Выборка.Следующий() Цикл
				...
				Иначе	
					...
					Если ОбменДаннымиЗагрузка = Истина Тогда
						...
						Исключение
							ЗаписьЖурналаРегистрации("Удаление документов", УровеньЖурналаРегистрации.Ошибка,,Выборка.Ссылка,"Ошибка пометки на удаление. Подробности: "+ОписаниеОшибки());
							Лог(тлог, "Ошибка пометкина удаление: "+строка(Выборка.СсылкаПредставление));
						КонецПопытки;
					Иначе
...
Процедура УдалитьРегистрыСведенийСервер()
	...
	Попытка
		...
		КонецЦикла;
	Исключение
		ВызватьИсключение "Ошибка при удалении из регистров сведений. Подробности: "+ОписаниеОшибки();
	КонецПопытки;
